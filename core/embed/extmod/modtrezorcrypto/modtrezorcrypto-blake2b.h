...
STATIC mp_obj_t mod_detahardcrypto_Blake2b_update(mp_obj_t self, mp_obj_t data);
...
///     """
STATIC mp_obj_t mod_detahardcrypto_Blake2b_make_new(const mp_obj_type_t *type,
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(mod_detahardcrypto_Blake2b_update_obj,
                                 mod_detahardcrypto_Blake2b_update);

/// def digest(self) -> bytes:
///     """
///     Returns the digest of hashed data.
///     """
STATIC mp_obj_t mod_detahardcrypto_Blake2b_digest(mp_obj_t self) {
  mp_obj_Blake2b_t *o = MP_OBJ_TO_PTR(self);
  BLAKE2B_CTX ctx = {0};
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mod_detahardcrypto_Blake2b_digest_obj,
                                 mod_detahardcrypto_Blake2b_digest);

STATIC mp_obj_t mod_detahardcrypto_Blake2b___del__(mp_obj_t self) {
  mp_obj_Blake2b_t *o = MP_OBJ_TO_PTR(self);
  memzero(&(o->ctx), sizeof(BLAKE2B_CTX));
  return mp_const_none;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mod_detahardcrypto_Blake2b___del___obj,
                                 mod_detahardcrypto_Blake2b___del__);

STATIC const mp_rom_map_elem_t mod_detahardcrypto_Blake2b_locals_dict_table[] = {
    {MP_ROM_QSTR(MP_QSTR_update),
     MP_ROM_PTR(&mod_detahardcrypto_Blake2b_update_obj)},
    {MP_ROM_QSTR(MP_QSTR_digest),
     MP_ROM_PTR(&mod_detahardcrypto_Blake2b_digest_obj)},
    {MP_ROM_QSTR(MP_QSTR___del__),
     MP_ROM_PTR(&mod_detahardcrypto_Blake2b___del___obj)},
...
STATIC MP_DEFINE_CONST_DICT(mod_detahardcrypto_Blake2b_locals_dict,
                            mod_detahardcrypto_Blake2b_locals_dict_table);

STATIC const mp_obj_type_t mod_detahardcrypto_Blake2b_type = {
    {&mp_type_type},
    .name = MP_QSTR_Blake2b,
    .make_new = mod_detahardcrypto_Blake2b_make_new,
    .locals_dict = (void *)&mod_detahardcrypto_Blake2b_locals_dict,
};
