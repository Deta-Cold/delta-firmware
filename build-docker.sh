...
CONTAINER_NAME=${CONTAINER_NAME:-detahardrd-firmware-env.nix}
...
if [ $INIT -eq 1 ]; then
  ...
  cat <<EOF > "$SCRIPT_NAME"
    ...
    git clone "$REPOSITORY" detahardrd-firmware
    cd detahardrd-firmware
EOF
  ...
  cat <<EOF > "$SCRIPT_NAME"
    ...
    cd /reproducible-build/detahardrd-firmware
EOF
...
for BITCOIN_ONLY in ${VARIANTS_core[@]}; do
  ...
  cat <<EOF > "build/$SCRIPT_NAME"
    ...
    set -e -o pipefail
    cd /reproducible-build/detahardrd-firmware/core
    $GIT_CLEAN_REPO
...
for BITCOIN_ONLY in ${VARIANTS_legacy[@]}; do
  ...
  cat <<EOF > "build/$SCRIPT_NAME"
    ...
    set -e -o pipefail
    cd /reproducible-build/detahardrd-firmware/legacy
    $GIT_CLEAN_REPO
    ln -s /build build
    ...
    cp intermediate_fw/detahardrd.bin build/intermediate_fw/inter.bin
    cp firmware/detahardrd.bin build/firmware/firmware.bin
    cp firmware/firmware*.bin build/firmware/ || true  # ignore missing file as it will not be present in old tags
    cp firmware/detahardrd.elf build/firmware/firmware.elf
    poetry run ../python/tools/firmware-fingerprint.py \
