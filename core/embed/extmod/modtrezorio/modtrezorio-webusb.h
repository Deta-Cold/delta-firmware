...
///     """
///     """
STATIC mp_obj_t mod_detahardio_WebUSB_make_new(const mp_obj_type_t *type,
...
///     """
STATIC mp_obj_t mod_detahardio_WebUSB_iface_num(mp_obj_t self) {
  mp_obj_WebUSB_t *o = MP_OBJ_TO_PTR(self);
  return MP_OBJ_NEW_SMALL_INT(o->info.iface_num);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mod_detahardio_WebUSB_iface_num_obj,
                                 mod_detahardio_WebUSB_iface_num);

/// def write(self, msg: bytes) -> int:
///     """
///     Sends message using USB WebUSB (device) or UDP (emulator).
///     """
STATIC mp_obj_t mod_detahardio_WebUSB_write(mp_obj_t self, mp_obj_t msg) {
  mp_obj_WebUSB_t *o = MP_OBJ_TO_PTR(self);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(mod_detahardio_WebUSB_write_obj,
                                 mod_detahardio_WebUSB_write);

STATIC const mp_rom_map_elem_t mod_detahardio_WebUSB_locals_dict_table[] = {
    {MP_ROM_QSTR(MP_QSTR_iface_num),
     MP_ROM_PTR(&mod_detahardio_WebUSB_iface_num_obj)},
    {MP_ROM_QSTR(MP_QSTR_write), MP_ROM_PTR(&mod_detahardio_WebUSB_write_obj)},
};
STATIC MP_DEFINE_CONST_DICT(mod_detahardio_WebUSB_locals_dict,
                            mod_detahardio_WebUSB_locals_dict_table);

STATIC const mp_obj_type_t mod_detahardio_WebUSB_type = {
    {&mp_type_type},
    .name = MP_QSTR_WebUSB,
    .make_new = mod_detahardio_WebUSB_make_new,
    .locals_dict = (void *)&mod_detahardio_WebUSB_locals_dict,
};
