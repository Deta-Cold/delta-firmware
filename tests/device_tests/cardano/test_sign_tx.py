...
from detahardlib import cardano, device, messages
...
def test_cardano_sign_tx_failed(client: Client, parameters, result):
    with pytest.raises(detahardFailure, match=result["error_message"]):
...
def _transform_expected_result(result):
    """Transform the JSON representation of the expected result into the format which is returned by detahardlib.
