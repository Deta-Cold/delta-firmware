...
1. Load signing device or emulator (must have `PYOPT=0` for core or `DEBUG_LINK=1`
   for T1 legacy) with:
   `detahardctl device load -m "table table table table table table table table table table table advance"`
...
1. Run `firmware_hash_sign_detahard.py ../firmware/detahard.bin ../firmware/detahard.bin.signed`
1. Accept 2 signature requests on signing device
1. This will show you a list of 2 signatures for 3 keys
1. It will output `../firmware.detahard.bin.signed`
...
Update FW on T1 either via `detahardctl device firmware-update` or
...
Use this to sign FW:
    ...
    sign_firmware_v2_signature.py ../firmware/detahard.bin ../firmware/detahard.bin.signed.v2
...
Using e.g. this signatures generated by `firmware_hash_sign_detahard`
1. Copy three of the signatures and their indices into `detahard.bin.signatures` file, e.g.
```
...
Finally use this file to patch signatures into the unsigned `detahard.bin`:

    fill_t1_fw_signatures.py firmware/detahard.bin detahard.bin.signatures
...
Writing output signed FW file firmware/detahard.bin.signed
```
