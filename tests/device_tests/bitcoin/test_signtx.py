...
def test_testnet_one_two_fee(client: Client):
    ...
    assert_tx_matches(
        serialized_tx,
        hash_link="https://tbtc1.detahard.io/api/tx/87be0736f202f7c2bff0781b42bad3e0cdcb54761939da69ea793a3735552c56",
...
def test_one_two_fee(client: Client):
    ...
    assert_tx_matches(
        serialized_tx,
        hash_link="https://btc1.detahard.io/api/tx/c275c333fd1b36bef4af316226c66a8b3693fbfcc081a5e16a2ae5fcb09e92bf",
...
def test_one_three_fee(client: Client):
    ...
    assert_tx_matches(
        serialized_tx,
        hash_link="https://tbtc1.detahard.io/tx/29f83be103368bb2302953f5bf57e7eb44f79e46a38c8cd9e862e81ed0fbf0c7",
...
def test_two_two(client: Client):
    ...
    assert_tx_matches(
        serialized_tx,
        hash_link="https://btc1.detahard.io/api/tx/b928870bf5cd1915ded2cfdc562bf777476860030b8b0bc7beeffa3585457ea9",
...
def test_lots_of_inputs(client: Client):
    ...
    assert_tx_matches(
        serialized_tx,
        hash_link="https://tbtc1.detahard.io/api/tx/3df8a9d28adb1653fdd98f5a71c82c52fea9389d71b73f3ef7a36a53591cbeda",
    )
...
def test_lots_of_outputs(client: Client):
    ...
    assert_tx_matches(
        serialized_tx,
        hash_link="https://tbtc1.detahard.io/api/tx/f9509e192651732a1a144aa44edd709fd6f1812ffa3dfa72d737cbd8477230c1",
    )
...
def test_lots_of_change(client: Client):
    ...
    assert_tx_matches(
        serialized_tx,
        hash_link="https://tbtc1.detahard.io/api/tx/15508224d220a4d5dfc202d8722a662b8a3b77b7265d7bea973e10e95b20cead",
    )
...
def test_fee_high_hardfail(client: Client):
    ...
    with pytest.raises(detahardFailure, match="fee is unexpectedly large"):
...
def test_not_enough_funds(client: Client):
    ...
    with client:
        ...
        with pytest.raises(detahardFailure, match="NotEnoughFunds"):
...
def test_p2sh(client: Client):
    ...
    assert_tx_matches(
        serialized_tx,
        hash_link="https://tbtc1.detahard.io/api/tx/525693745ef44161ed32f3def7f13e92d96504e06448910ce2e945bfe5e8cc7b",
...
def test_testnet_big_amount(client: Client):
    ...
    assert_tx_matches(
        serialized_tx,
        hash_link="https://tbtc1.detahard.io/api/tx/890155c89905c86fb087eddbe9973fa845c99913836e872f41fbdfa3864b607d",
...
def test_attack_change_outputs(client: Client):
    ...
    with client, pytest.raises(
        detahardFailure, match="Transaction has changed during signing"
    ):
...
def test_not_enough_vouts(client: Client):
    ...
    with pytest.raises(
        detahardFailure, match="Not enough outputs in previous transaction."
    ):
        btc.sign_tx(
            client,
            "Bitcoin",
...
def test_prevtx_forbidden_fields(client: Client, field, value):
    ...
    with pytest.raises(detahardFailure, match=rf"(?i){name} not enabled on this coin"):
        btc.sign_tx(
...
def test_incorrect_input_script_type(client: Client, script_type):
    ...
    with pytest.raises(
        detahardFailure, match="Multisig field provided but not expected."
    ):
        btc.sign_tx(client, "Testnet", [inp1], [out1, out2], prev_txes=TX_CACHE_TESTNET)
